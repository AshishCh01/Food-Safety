...
def test_ingest_embedding_failure_leaves_no_partial_chunks(db_session: Session, monkeypatch) -> None:
    ...
    # A tiny target_chars keeps each heading's body over the merge threshold
    # on its own, so this document deterministically chunks into two pieces
    # (exercising the >=2 embed calls this test needs) regardless of the
    # small-section-merging behaviour in app/rag/chunking.py, which is only
    # meant to combine sections that are still small relative to the real
    # (much larger) default target_chars.
    monkeypatch.setattr(ingestion.chunking, "get_settings", lambda: _FakeChunkingSettings(target_chars=10, overlap_chars=2))
...
def test_reingest_replaces_existing_chunks(db_session: Session, monkeypatch) -> None:
    monkeypatch.setattr(ai_service, "embed_text", _fake_embedding)
    # See test_ingest_embedding_failure_leaves_no_partial_chunks above - a
    # tiny target_chars keeps "# A"/"# B" from being merged into one chunk,
    # so the second ingest deterministically produces 2 chunks instead of
    # the small-section-merging behaviour collapsing them into 1.
    monkeypatch.setattr(ingestion.chunking, "get_settings", lambda: _FakeChunkingSettings(target_chars=10, overlap_chars=2))
